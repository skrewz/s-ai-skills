...
func NewSearcher(scraperImpl *scraper.Scraper, logger *slog.Logger) *Searcher {
	if scraperImpl == nil {
		scraperImpl = scraper.NewScraper("", logger)
	}
	if logger == nil {
...
func newTestSearcher(baseURL string, scraperImpl *scraper.Scraper, logger *slog.Logger) *Searcher {
	if scraperImpl == nil {
		scraperImpl = scraper.NewScraper("", logger)
	}
	if logger == nil {
...
func (s *Searcher) Search(ctx context.Context, query string) ([]SearchResult, error) {
	...
	s.logger.Debug("Searching", "query", query, "url", searchURL)
	...
	s.logger.Debug("Search complete", "query", query, "results", len(results))
...
func isAdURL(u string) bool {
	...
	return strings.Contains(u, "duckduckgo.com/web/")
}
