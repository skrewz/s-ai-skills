...
func TestSearch_Integration(t *testing.T) {
	...
	searcher := newTestSearcher(testServer.URL, nil, nil)
...
func TestParseSearchResults_ValidNonAdURLsAllowed(t *testing.T) {
	html := `
	<html>
		<body>
			<div class="result">
				<h2><a class="result__a" href="https://go.dev/">Go Language</a></h2>
				<a class="result__snippet">The Go Programming Language</a>
			</div>
			<div class="result">
				<h2><a class="result__a" href="//duckduckgo.com/l/?uddg=https%3A%2F%2Fgolang.org%2Fdoc">Go Documentation</a></h2>
				<a class="result__snippet">Go documentation and tutorials.</a>
			</div>
			<div class="result">
				<h2><a class="result__a" href="https://github.com/golang/go">Go on GitHub</a></h2>
				<a class="result__snippet">Go source code on GitHub.</a>
			</div>
		</body>
	</html>
	`
	...
	if results[0].Title != "Go Language" {
		t.Errorf("expected title %q, got %q", "Go Language", results[0].Title)
	}
	if results[0].URL != "https://go.dev/" {
		t.Errorf("expected URL %q, got %q", "https://go.dev/", results[0].URL)
	}
	if results[1].Title != "Go Documentation" {
		t.Errorf("expected title %q, got %q", "Go Documentation", results[1].Title)
	}
	if results[1].URL != "//duckduckgo.com/l/?uddg=https%3A%2F%2Fgolang.org%2Fdoc" {
		t.Errorf("expected URL %q, got %q", "//duckduckgo.com/l/?uddg=...", results[1].URL)
	}
	if results[2].Title != "Go on GitHub" {
		t.Errorf("expected title %q, got %q", "Go on GitHub", results[2].Title)
	}
}
...
func TestParseSearchResults_AdURLsFiltered(t *testing.T) {
	html := `
	<html>
		<body>
			<div class="result">
				<h2><a class="result__a" href="https://example.com/organic">Organic Result</a></h2>
				<a class="result__snippet">This is an organic result.</a>
			</div>
			<div class="result">
				<h2><a class="result__a" href="//duckduckgo.com/web/?uddg=https%3A%2F%2Fads.example.com%2Fclick">Ad Result</a></h2>
				<a class="result__snippet">This is an ad.</a>
			</div>
			<div class="result">
				<h2><a class="result__a" href="https://example.com/another">Another Organic</a></h2>
				<a class="result__snippet">Another organic result.</a>
			</div>
		</body>
	</html>
	`
	...
	if len(results) != 2 {
		t.Fatalf("expected 2 results (ads filtered), got %d", len(results))
	}

	if results[0].Title != "Organic Result" {
		t.Errorf("expected title %q, got %q", "Organic Result", results[0].Title)
	}
	if results[1].Title != "Another Organic" {
		t.Errorf("expected title %q, got %q", "Another Organic", results[1].Title)
	}
}
...
func TestIsAdURL(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "duckduckgo organic redirect",
			url:      "//duckduckgo.com/l/?uddg=https%3A%2F%2Fexample.com",
			expected: false,
		},
		{
			name:     "duckduckgo web redirect",
			url:      "//duckduckgo.com/web/?uddg=https%3A%2F%2Fexample.com",
			expected: true,
		},
		{
			name:     "normal url",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := isAdURL(tt.url)
			if result != tt.expected {
				t.Errorf("isAdURL(%q) = %v, want %v", tt.url, result, tt.expected)
			}
		})
	}
}
